...
# loss generator for back propagation
def calc_loss(batch, train_net, target_net, gamma, device="cpu"):
    ...
    state_action_values = (
        train_net(states_v).gather(1, actions_v.unsqueeze(-1)).squeeze(-1)
    )
    next_state_actions = train_net(next_states_v).max(1)[1]
    next_state_values = (
        target_net(next_states_v)
