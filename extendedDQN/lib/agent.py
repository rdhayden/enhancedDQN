...
class Agent:
    ...
    def _reset(self):
        # start a new episode and reset health
        self.game.new_episode(self.record_path)
        self.health = self.game.get_state().game_variables
    ...
    # Down-samples the image and stack the frame
    ...
    def play_step(self, play_random, device="cpu", write_to_buffer=True):
        ...
        if play_random:
            ...
            # add one dimension to the array, make it a tensor and generate q value
            state_a = np.array([self.state], copy=False)
            state_tensor = torch.tensor(state_a, dtype=torch.float).to(device)
            q_vals_tensor = self.net(state_tensor)
            # get the index of the action predicted with the highest value
            _, index = torch.max(
                q_vals_tensor, 1
            )
        ...
        if is_done:
            ...
            new_state = (
                self.state
            )  # if done, it will get overriden later anyway so this is just a placeholder
            exp = Experience(self.state, action, reward, is_done, new_state)
            if write_to_buffer:
                self.exp_buffer.append(exp)
            self._reset()
        else:
            # apply health penalty if applicable because it was dificult to modify the ZDoom scenario directly with this penalty
            if self.health_reward > 0:
                next_health = self.game.get_state().game_variables
                if next_health < self.health:
                    reward -= self.health_reward
